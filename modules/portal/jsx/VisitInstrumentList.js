            ...
            instruments = this.state.instruments.map((instrument) => {
                ...
                const ScanQR = (testname, linkURI) => {
                    const onScan = (value) => {
                        ...
                        try {
                            ...
                            if (obj.PSCID !== this.props.Candidate.Meta.PSCID
                                ...
                                swal.fire({
                                    title: testname,
                                    icon: 'info',
                                    html: '<p>You will be directed to ' + testname + ' in a new window. Once you are finished, return here to continue with your visit.</p>'
                                }).then(() => {
                                    window.open(linkURI)
                                }
                                );
                    ...
                    return () => {
                        this.setState({'QRModal':
                            <QRModal key={testname}
                ...
                return (<tr key={instrument.Test_name}>
                    <td style={{textAlign: 'left'}}>
                        <span href={linkURI}>{instrument.Test_name}</span>
                    </td>
                    <td>
                        <progress
                    ...
                    <td><button onClick={ScanQR(instrument.Test_name, linkURI)}>Scan to access</button></td>
                    </tr>);
            });
