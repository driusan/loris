...
if (isset($_REQUEST['p_status'])) {
    print " \n";
    $results = $DB->pselect(
        "SELECT Description FROM participant_status_options WHERE parentID=:pid",
    ...
    $results = $DB->pselect(
        "SELECT participant_status, participant_suboptions
    ...
    foreach ($results as $row) {
        ...
        $desc     = $DB->pselectOne(
            "SELECT Description FROM participant_status_options WHERE ID=:id",
            array('id' => $id)
        );
        $sub_desc = $DB->pselectOne(
            "SELECT Description FROM participant_status_options WHERE ID=:sid",
